...
int main(int argc,char* argv[]) {
    ...
    {
        struct resampler r;
        ASSERT( BarycentricAVXinit  (&r,src_shape,dst_shape,3));
        ASSERT( BarycentricAVXsource(&r,src));
        ASSERT( BarycentricAVXdestination(&r,dst));
        ASSERT( BarycentricAVXresample(&r,cube,0,0));  // iterate manually over (&4,cube,[0,90],[0,1])
        ASSERT( BarycentricAVXresult(&r,dst));
