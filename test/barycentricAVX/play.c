...
int main(int argc,char* argv[]) {
    ...
    {
        struct resampler r;
        TIME(ASSERT( BarycentricAVXinit  (&r,src_shape,dst_shape,3)));
        TIME(ASSERT( BarycentricAVXsource(&r,src)));
        TIME(ASSERT( BarycentricAVXdestination(&r,dst)));

        //TicTocTimer t=tic();
        TIME(ASSERT( BarycentricAVXresample(&r,cube,90,1)));  // iterate manually over (&4,cube,[0,90],[0,1])
        //printf("TIME %fs\n",toc(&t));

        TIME(ASSERT( BarycentricAVXresult(&r,dst)));
                BarycentricAVXrelease(&r);
    }
